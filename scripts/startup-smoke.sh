...
for html_name, html in [('index.html', root), ('hazama-index.html', alt)]:
    for asset in [
        'hazama-style.css?v=2.38',
        'hazama-seed.js?v=2.38',
        'hazama-state.js?v=2.38',
        'hazama-gate-run.js?v=2.38',
        'hazama-main.js?v=2.38',
    ]:
...
assert 'Hazama main.js v2.38' in js, 'hazama-main.js バージョンが期待値ではありません'
...
assert 'hazama-pwa-v2.38' in sw and 'PRECACHE_URLS' in sw and 'hazama-depths.json' in sw and 'CACHE_PREFIX' in sw, 'service worker のPWA cache設定が見つかりません'
...
assert 'browser first playable smoke passed' in browser_first_playable and 'optional Playwright' in browser_first_playable and 'won story Ω option should be enabled' in browser_first_playable and 'PWA manifest link is missing' in browser_first_playable and 'hazama-pwa-v2.38' in browser_first_playable, 'browser first playable smoke script が見つかりません'
...
assert '次にやること' in js and 'hz-loop-priority' in js and 'BGM: 同じ画面推奨' in js, 'v2.38 next-action guide が見つかりません'
