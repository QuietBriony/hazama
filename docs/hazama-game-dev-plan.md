...
- Next merge goal: verify the v2.44 post-story flow, Gate Run pulse cues, and same-screen BGM in a real browser/device pass, especially next-action clarity, mobile touch hierarchy, install/offline reload, Breath Gate hierarchy, outcome panels, and the `A_reborn` run record.
