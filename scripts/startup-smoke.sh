...
for html_name, html in [('index.html', root), ('hazama-index.html', alt)]:
    for asset in [
        'hazama-style.css?v=2.24',
        'hazama-seed.js?v=2.24',
        'hazama-state.js?v=2.24',
        'hazama-gate-run.js?v=2.24',
        'hazama-main.js?v=2.24',
    ]:
...
assert 'Hazama main.js v2.24' in js, 'hazama-main.js バージョンが期待値ではありません'
...
assert 'GATE_RUN_TURN_LIMIT: 14' in gate, 'v2.24 Gate Run のターン制限が見つかりません'
...
assert 'H境界: 落ち着き' in js and 'N境界: 響き' in js and '道を選ぶ' in js and 'Gate Run / 扉操作' in js, 'v2.24 balance UI labels が見つかりません'
...
assert '落ち着き' in js and '響き' in js and '立て直し中' in js, 'v2.24 の説明語彙が見つかりません'
