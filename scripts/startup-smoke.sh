...
for html_name, html in [('index.html', root), ('hazama-index.html', alt)]:
    for asset in [
        'hazama-style.css?v=2.41',
        'hazama-seed.js?v=2.41',
        'hazama-state.js?v=2.41',
        'hazama-gate-run.js?v=2.41',
        'hazama-main.js?v=2.41',
    ]:
...
assert 'Hazama main.js v2.41' in js, 'hazama-main.js バージョンが期待値ではありません'
...
assert 'hazama-pwa-v2.41' in sw and 'PRECACHE_URLS' in sw and 'hazama-depths.json' in sw and 'CACHE_PREFIX' in sw, 'service worker のPWA cache設定が見つかりません'
...
assert 'hz-rogue-hud' not in js and 'DEPTH MAP' not in js and 'RUN LOG' not in js, 'v2.41 HUD集約: 旧roguelike HUD markup が撤去されていません'
assert 'hz-rogue-hud' not in css and 'hz-map-tile' not in css, 'v2.41 HUD集約: 旧roguelike HUD CSS が撤去されていません'
assert 'function renderSurfaceReturn' in js and 'function updateBackdrop' in js, 'v2.41 浮上終端(二極)/背景クロスフェード退場が見つかりません'
assert 'id="hz-gate"' in root and '.hz-gate ' in css, 'v2.41 沈下HUDへ集約した扉(GATE)計器が見つかりません'
...
assert 'hz-start-guide' in js and '最初にやること' in js and 'まず進む' in js and 'hz-start-guide' in css, 'v2.35 first screen guidance が見つかりません'
assert 'hz-decision-panel' in js and '本文を読んでから選ぶ' in js and 'hz-decision-panel' in css and 'route-options-host' in js, 'v2.36 post-story decision panel が見つかりません'
...
assert '次周回開始 / Ωは閉じた' in js and 'resetWon' in gate and '次周回: HUBへ戻るとΩは閉じる' in js, 'v2.36 next-loop Ω close が見つかりません'
...
assert 'browser first playable smoke passed' in browser_first_playable and 'optional Playwright' in browser_first_playable and 'won story Ω option should be enabled' in browser_first_playable and 'PWA manifest link is missing' in browser_first_playable and 'hazama-pwa-v2.41' in browser_first_playable, 'browser first playable smoke script が見つかりません'
...
assert '同じ画面で鳴らす' in js and 'startInlineBgm' in js and 'createMediaStreamDestination' in js and 'MediaMetadata' in js, 'v2.37 same-screen BGM engine が見つかりません'
assert 'リンクでMusicを開く' in js and '同じ画面BGM待ち' in js and 'スマホは同じ画面再生を推奨' in js and 'data-bgm-phase="blocked"' in css, 'BGM popup blocked fallback が見つかりません'
assert 'hz-bgm-inline' in css and 'data-bgm-phase="inline"' in css and 'data-inline-bgm' in js, 'v2.37 inline BGM UI/CSS が見つかりません'
...
assert 'hz-choice-main' in js and 'hz-choice-open' in js and 'flashChoiceReaction' in js, 'v2.41 等価な選択肢(脱telegraph)＋選択後reactionが見つかりません'
assert '次にやること' in js and 'hz-loop-priority' in js and 'BGM: 同じ画面推奨' in js, 'v2.41 next-action guide が見つかりません'
assert '今のノリ' in js and 'hz-gate-pulse' in js and 'おすすめ' in js and 'hz-gate-action--recommended' in css, 'v2.41 playful Gate Run polish が見つかりません'
assert 'const HazamaAtmos' in js and 'function applyAtmosphere' in js and 'inlineBgmGlitchHit' in js, 'v2.41 統合アート/音(applyAtmosphere/glitch-synced BGM)が見つかりません'
assert 'function updateSinkHud' in js and 'id="hz-garden"' in root and 'id="hz-sink-hud"' in root and 'data-text="Hazama"' in root, 'v2.41 沈下HUD/反転ガーデン/かすれタイトルが見つかりません'
assert '.hz-garden' in css and '.hz-sink-fill' in css and 'hzPeel' in css, 'v2.41 統合アートCSS(garden/sink/peel)が見つかりません'
