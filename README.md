...
`index.html` と `hazama-index.html` は `hazama-seed.js?v=2.17`、`hazama-state.js?v=2.17`、`hazama-main.js?v=2.17` の順に読み込みます。
...
- `Music` は Hazama profile payload を URL hash に載せて Music を開く
...
- `MusicでSTART.HZM` / dev `LOCAL` クリック時は、その選択先を優先して開く
- `Music` 表示は未接続 / 待機中 / 再生中を短く出す
- dev操作は localhost の drawer へ退避する
- Hazama起動後の最初のクリック / タップ / キー入力で `MusicでSTART.HZM` と同じMusic起動を試みる
...
- `index.html` が `hazama-main.js?v=2.17` を参照している
- `hazama-main.js` が `v2.17` で配信される
