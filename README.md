...
`index.html` と `hazama-index.html` は `hazama-seed.js?v=2.14`、`hazama-state.js?v=2.14`、`hazama-main.js?v=2.14` の順に読み込みます。

## v2.14 Audio Auto-Resume + Visual Separation

各深度では、既存の選択肢ナビゲーションに加えて以下が動きます。

- 初期/HUBでは探索者キービジュアルだけを出し、通常深度以降は曼荼羅ゲートだけを出す
- 深度や共鳴が進むほど、曼荼羅の存在感と回転・発光が少し強まる
- Gate Intelligence が現在の目的、危険度、推奨ルート、ゲート開度を提示する
- Gate Run が `潜る / 観測 / 調律 / 同期 / 退避` の5行動でゲート開度100%を目指す
- Ω到達が本筋のゴール。Ωは `GATE OPEN` 後に入れる
- `GATE OPEN` で達成、安定切れやターン超過で `COLLAPSED` になりHUBへ退避する
...
- `index.html` が `hazama-main.js?v=2.14` を参照している
- `hazama-main.js` が `v2.14` で配信される
