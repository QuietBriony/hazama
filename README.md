...
v2.45 は「機能説明画面っぽい」を解消。本文直下は「どう読む」選択肢だけを主役に出し、説明/操作の足場(最初にやること・初回ループguide・Gate Run・BGM・セッション・Breath Gate)を折りたたみドロワー(既定で閉)へ退避。没入の読書体験に絞り、ゲーム操作はドロワーで保持。

- runtime asset更新に合わせて `v2.45` と `hazama-pwa-v2.45` へ同期

## v2.44 Immersive Layout Unify (PC/モバイル没入統一)

v2.44 は「スマホは没入(claude版)だがPCは機能説明画面っぽい」差を解消。PCの960px幅ダッシュボード化をやめ、コンテンツを640pxの没入読書カラムに統一。全画面アートが左右に呼吸し、PCでもスマホと同じ没入の見えに。

- runtime asset更新に合わせて `v2.44` と `hazama-pwa-v2.44` へ同期

## v2.43 Immersive Art-Forward v2 (アート視認性ブースト)

v2.43 は v2.42 の没入化を強化。モバイルでガラスパネルが画面を覆い、薄い線画ガーデンが暗いガラス越しに沈んで見えた問題へ：ガーデンに鮮烈な大気ウォッシュ(シアン→紫→マゼンタ)を敷き常時アートを立たせ、各パネルのガラスを明るく(alpha↓/blur↓)してアートを透かす。本文は text-shadow で可読維持。

- runtime asset更新に合わせて `v2.43` と `hazama-pwa-v2.43` へ同期

## v2.42 Immersive Art-Forward (没入アート前面化)

v2.42 は実機FB「アートが埋もれて codex 的」への対応。背景アート(descent写真/反転ガーデン/曼荼羅/グリッジ)を表紙(anchor)含め全画面化し、本文/パネルを半透明ガラス(backdrop-blur)にしてアートを透かす＝slice級の没入を回復。浅でもアートが立つ可視カーブ。本文可読性は dark glass + blur で維持。

- runtime asset更新に合わせて `v2.42` と `hazama-pwa-v2.42` へ同期

## v2.41 Integration Polish (統合版・本番化前ポリッシュ)

v2.41 は統合版（slice→本体畳み込み）の実機フィードバック6点を反映した手触り調整です（master/本番は無変更）。

- 選択肢を視覚的に等価・読みやすく（kind別マーク/機械的subを撤去）し、認識判定は選んだ後の reaction に。分岐の自由度も拡張
- 終端を二極化：核へ降りる(没入) ⇄ 浮上して帰る(帰還)。Ω未達は GAME OVER でなく「浮上」という有効な結末へ（Ω到達は attuned のみ・ハード維持）
- descent写真の背景を沈下に従ってクロスフェード退場し、深部は完全に反転ガーデンへ置換
- 反転重森ガーデンを遠近/レイヤー/パララックス＋緻密ディテールで立体化（rAFなし＝モバイル軽量維持）
- 外殻を越えた後にサイレン的な"圧"のピークを inline BGM へ（浅は据え置き・balance重視）
- 旧ローグライク HUD を撤去し、沈下HUD(深度/観測者/認識/扉)へ計器を一本化（startup-smoke 旧アサーションは新サーフェスへ移行）
- runtime asset更新に合わせて `v2.41` と `hazama-pwa-v2.41` へ同期
- public route、storage key、`hazama-depths.json` schema、Music payload shape、Gate Run 数値、外部音源なしの静的Web方針は維持

## v2.40 Playful Gate Run Polish

v2.40 では、既存 loop と数値バランスを変えず、Gate Run の「今どれを押すと気持ちいいか」を読みやすくしました。

- Gate Run に `今のノリ` pulse を追加し、開門、退避、合わせどき、攻めどきなどの流れを短く表示
- 現在の状態に合う Gate Run ボタンへ `おすすめ` badge を出し、5つの行動がただ横並びに見えないようにする
- `A_reborn` の完了コピーに一周の手触りを出し、次周回へのフックを少し強める
- runtime asset更新に合わせて `v2.40` と `hazama-pwa-v2.40` へ同期
- public route、storage key、`hazama-depths.json` schema、Music payload shape、Gate Run 数値、外部音源なしの静的Web方針は維持

## v2.38 Gameplay Consistency Polish

v2.38 では、既存 first playable を壊さず、本文後の `展開` パネル内で「次にやること」と操作の優先順位を常に読めるようにしました。

- 初回ループ guide を `展開` パネルへ常時表示し、現在位置、次の行動、主導線、補助、BGM推奨をまとめて読む
- Gate Run / 道を選ぶ / Breath Gate / BGM の優先順位を画面内で明示し、Breath Gate と BGM は補助に留める
- `scripts/hazama-consistency-smoke.mjs` を追加し、depth graph、first playable route、Ω lock/relock marker、docs/runtime version drift を依存なしで検証
- runtime asset更新に合わせて `v2.38` と `hazama-pwa-v2.38` へ同期
- public route、storage key、`hazama-depths.json` schema、Music payload shape、外部音源なしの静的Web方針は維持

## v2.37 Same-Screen BGM

v2.37 では、スマホでゲーム画面へ戻ると別タブMusicのBGMが止まりやすい問題を避けるため、Hazama本体に小さな生成BGM rendererを追加しました。

- `BGM` チップまたは `同じ画面で鳴らす` で、ユーザー操作後に同じページ内の Web Audio を開始
- `hazama-profile` の sanitized `audio` / `ucm` / `source.stage` からテンポ、密度、明るさ、低域、簡易フレーズを生成
- iOS / Bluetooth route 用に、可能な環境では Web Audio を hidden media stream bridge にも流す
- 外部 `QuietBriony/Music` タブ導線、payload shape、raw Breath Gate非保存、音源/サンプル非追加方針は維持

## v2.36 Story-First Loop Polish

v2.36 では、実機プレイの違和感を受けて、Hazamaを「本文を読む -> 展開パネルで選ぶ」流れへ寄せました。

- Gate Run、道の選択、Breath Gate、セッション操作を本文後の単一 `展開` パネルに集約
- `A_reborn` 後にHUBへ戻るとΩを閉じ、次周回はGate Runをもう一度開く
- BGMはスマホで別タブ復帰時に止まることがあるため、自動タブ起動をやめて任意 companion として扱う
- public route、storage key、Music payload shape、外部音源なしの静的Web方針は維持
...
Hazama は音源ファイルやサンプルを保持せず、現在の展開から音楽 profile を生成します。いまは `QuietBriony/Music` を外部 active provider として扱い、`namima` と `drum-floor` は future provider として後から差し込める構造にしています。
...
- Music 側に `#hazama=<payload>` boot と `postMessage` receiver があれば、外部Musicの自動生成音へ接続可能
- Hazama本体は v2.37 から同じ画面の lightweight Web Audio BGM を持ち、スマホではこちらを推奨する
- 外部Musicタブが開いてSTART.HZM済みのときは、従来通りMusic側の再生へも追従できる
...
`hazama-check` は autonomy docs、PWA static contract、Hazama consistency、balance、first playable、startup、optional browser smoke を順番に実行します。Playwrightが無い環境では browser/localStorage 系 smoke だけskipし、他が通れば `0 FAIL` です。
...
`hazama-consistency-smoke` は depth graph、first playable route、Ω lock/relock marker、runtime/docs version drift、展開パネルの次アクション導線を依存なしで確認します。
...
- `index.html` が `hazama-gate-run.js?v=2.45` と `hazama-main.js?v=2.45` を参照している
- `hazama-main.js` が `v2.45` で配信される
