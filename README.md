...
`index.html` と `hazama-index.html` は `hazama-seed.js?v=2.30`、`hazama-state.js?v=2.30`、`hazama-gate-run.js?v=2.30`、`hazama-main.js?v=2.30` の順に読み込みます。
...
- 初回ループは `A_start -> HUB -> Gate Run -> Breath Gate -> Ω unlock -> Ω -> A_reborn` を目標にする
- `Gate Intelligence` に現在の段階、次の目標、扉100%までの道筋を短く表示する
- 選択肢は「足元の光をたどって先へ進む」のような普通の行動文として表示する
- 落ち着きや扉の開きなどの効果は、選択肢本文と分けて小さく表示する
- 一言入力は必須進行ではなく、`ひと息置く` として落ち着きを戻す限定回復にする。連続使用は効きが落ちる
...
- `index.html` が `hazama-gate-run.js?v=2.30` と `hazama-main.js?v=2.30` を参照している
- `hazama-main.js` が `v2.30` で配信される
...
- `ひと息置く`: 問いに一言置き、沈黙後に落ち着きを得る。連続使用では響きや扉の効率が落ちる
