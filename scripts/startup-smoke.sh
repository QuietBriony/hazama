...
for html_name, html in [('index.html', root), ('hazama-index.html', alt)]:
    for asset in [
        'hazama-style.css?v=2.21',
        'hazama-seed.js?v=2.21',
        'hazama-state.js?v=2.21',
        'hazama-main.js?v=2.21',
    ]:
...
assert 'Hazama main.js v2.21' in js, 'hazama-main.js バージョンが期待値ではありません'
...
assert 'GATE_RUN_TURN_LIMIT = 14' in js, 'v2.21 Gate Run のターン制限が見つかりません'
assert 'GATE_SYNC_READY_RESONANCE = 18' in js and 'GATE_SYNC_READY_CHARGE = 45' in js and 'GATE_SYNC_MARK_CHARGE = 35' in js and '準備OK' in js, 'Gate Run の sync 準備表示が見つかりません'
...
assert 'H境界: 落ち着き' in js and 'N境界: 響き' in js and '道を選ぶ' in js and 'Gate Run / 扉操作' in js, 'v2.21 balance UI labels が見つかりません'
...
assert '落ち着き' in js and '響き' in js and '立て直し中' in js, 'v2.21 の説明語彙が見つかりません'
