...
| ローグライク HUD（FLOOR/CALM/SYNC…） | **縮小（差し替え）** | 常時計器は隠す/縮小。沈下ゲージ＋戻り道＋観測者カウンタへ。**smoke が `hz-rogue-hud` 等を要求するため markup は残し非表示化＋smoke 拡張**（§8） |
...
> 注：これは Gate Run の**削除でなく再解釈**。M2-spine §5.5「残す→再解釈」に忠実。smoke が要求する
> Gate Run の文字列群（§8）は残しつつ、認識ゲート層を上に重ねる。
...
方針:
1. **既存機能の markup は可能な限り残す**（非表示クラスで縮小表示）＝ smoke 文字列を温存しつつ体験は刷新。
2. 壊さざるを得ない文字列は、**意味を保ったまま smoke 側を更新**（"必要なら拡張" の許可範囲）。削除でなく
   「縮小/再解釈に合わせた新文言」へ差し替え、対応する新アサーションを追加。
3. **新アサーションを足す**：depthMeta スキーマ（`depthMeta`/`deep`/`kind:"surface"`）、Route/legacy
   ...
   peel タイトル。新規 `scripts/depth-graph-smoke.mjs` / `route-smoke.mjs` を追加候補。
...
- **増分0（本書）**：`docs/INTEGRATION.md` 作成。`claude/integration` ブランチ。**[このコミット]**
- **増分1（スキーマ土台）**：`hazama-depths.json` に `depthMeta` を後方互換で導入（まず数ノード）。
  consistency smoke に depthMeta 許容を追加。挙動不変を確認。
- **増分2（データ転記：G〜Z＋Ω＋reborn の 1:1）**：衝突しない領域から slice 本文/voice/options を転記。
  maps_to 逆引き。first-playable 不変。
- **増分3（オフセット解消：zero/A→A_start, B〜F）**：§3 の編集マージ。**ユーザーへ対応表報告**。
- **増分4（Route/legacy/det_*/holds）**：分岐エンジンを本体へ。det_*/holds/drift をノード反映。route-smoke 追加。
- **増分5（認識ゲート）**：Gate Run 再解釈＋attunement＋surface deflection＋Ω は attuned 必須。
  hazama-gate-run.js 再解釈。balance smoke 整合。
...
- 本番 Pages（現 Codex 公開）と **別**の integration プレビューに出す。候補：
  - 既存 `hazama-preview`（QuietBriony/hazama-preview, main）に統合版を出す（[memory: hazama-preview-deploy] の手順）。
  - もしくは新規 integration 専用プレビュー repo（slice プレビューと混ざらないよう分離が望ましい場合）。
- どちらにするかは増分8 着手時にユーザーへ確認（slice プレビューを上書きしてよいか／別口にするか）。
- 反映確認は cache-bust 実体チェック必須。**master / 本番 hazama は無変更**。

---

## 付録：当面のユーザー確認事項（メッセージで挙げる）

1. ノードID対応 §3：方針β（骨格温存・編集マージ）でよいか。F 集約・zero/A 統合の編集判断。
2. 認識ゲート §5：Gate Run 資源ミニゲームを「縮小して認識ゲートの下層に残す」方針でよいか
   （完全撤去でないのは smoke 契約と "残す→再解釈" のため）。
3. プレビュー §12：既存 hazama-preview を統合版で上書きしてよいか／別口プレビューにするか。
4. 核（Ωより深い層）の供給：A=生ログ / **B=創作（現デフォルト）** / C=ハイブリッド（M2-spine §5 継続）。
</content>
</invoke>
