...
for html_name, html in [('index.html', root), ('hazama-index.html', alt)]:
    for asset in [
        'hazama-style.css?v=2.45',
        'hazama-seed.js?v=2.45',
        'hazama-state.js?v=2.45',
        'hazama-gate-run.js?v=2.45',
        'hazama-main.js?v=2.45',
    ]:
...
assert 'Hazama main.js v2.45' in js, 'hazama-main.js バージョンが期待値ではありません'
...
assert 'hazama-pwa-v2.45' in sw and 'PRECACHE_URLS' in sw and 'hazama-depths.json' in sw and 'CACHE_PREFIX' in sw, 'service worker のPWA cache設定が見つかりません'
...
assert 'hz-rogue-hud' not in js and 'DEPTH MAP' not in js and 'RUN LOG' not in js, 'v2.45 HUD集約: 旧roguelike HUD markup が撤去されていません'
assert 'hz-rogue-hud' not in css and 'hz-map-tile' not in css, 'v2.45 HUD集約: 旧roguelike HUD CSS が撤去されていません'
assert 'function renderSurfaceReturn' in js and 'function updateBackdrop' in js, 'v2.45 浮上終端(二極)/背景クロスフェード退場が見つかりません'
assert 'id="hz-gate"' in root and '.hz-gate ' in css, 'v2.45 沈下HUDへ集約した扉(GATE)計器が見つかりません'
...
assert 'browser first playable smoke passed' in browser_first_playable and 'optional Playwright' in browser_first_playable and 'won story Ω option should be enabled' in browser_first_playable and 'PWA manifest link is missing' in browser_first_playable and 'hazama-pwa-v2.45' in browser_first_playable, 'browser first playable smoke script が見つかりません'
...
assert 'hz-choice-main' in js and 'hz-choice-open' in js and 'flashChoiceReaction' in js, 'v2.45 等価な選択肢(脱telegraph)＋選択後reactionが見つかりません'
assert '次にやること' in js and 'hz-loop-priority' in js and 'BGM: 同じ画面推奨' in js, 'v2.45 next-action guide が見つかりません'
assert '今のノリ' in js and 'hz-gate-pulse' in js and 'おすすめ' in js and 'hz-gate-action--recommended' in css, 'v2.45 playful Gate Run polish が見つかりません'
assert 'const HazamaAtmos' in js and 'function applyAtmosphere' in js and 'inlineBgmGlitchHit' in js, 'v2.45 統合アート/音(applyAtmosphere/glitch-synced BGM)が見つかりません'
assert 'function updateSinkHud' in js and 'id="hz-garden"' in root and 'id="hz-sink-hud"' in root and 'data-text="Hazama"' in root, 'v2.45 沈下HUD/反転ガーデン/かすれタイトルが見つかりません'
assert '.hz-garden' in css and '.hz-sink-fill' in css and 'hzPeel' in css, 'v2.45 統合アートCSS(garden/sink/peel)が見つかりません'
