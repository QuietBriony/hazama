...
- shipped    : 表紙でJapanese / English (trial)を選ぶ表示専用Localeと`locales/en.json`を追加。
  初回の身体13ノード＋zero_holdの計14ノード、共有終端・設定・操作・記録、正答/誤答の全29エコー断片を英訳。
  英語の文字増加だけで演出待ちが伸びないよう、段落の文字送り時間を原文相当以下にする。
  短画面の入口と英文の改行を整え、拡大時にも結末の二択と画像保存が収まる短い操作文へ調整。
  未翻訳の本文/道を案内し、原文と日本語langを保持。ゲームの選択肢を削除/制限しない。
- scope      : 言語は開始前に選び、再読み込みで日本語へ戻す。言語操作では記憶を書かない/消さない。
  public route / hazama_spiral_v1 / depths schema / 原文72ノード / バランス / seed / 音エンジンは不変。
  保存キー・依存・音源・解析送信の追加なし。index/runtime/SW/catalogはe43同期、英語JSONはoptional precache。
  他の道・寄り道・周回変奏・無限深度・共有画像/共有文には日本語が残る。全編英語対応とは扱わない。
- checks     : `node scripts/hazama-check.mjs`: 2 PASS / 0 FAIL / 0 SKIP。slice/SWの構文、`git diff --check` PASS。
  新しい`reading-locale-smoke`はproduction Localeで14ノードの訳抜け、全エコー候補、原文不変、
  言語往復、未訳fallback、表示時間上限、版同期を検証。既存の二重選択/全文表示/音量/設定回帰もPASS。
- browser    : 実Chromiumで日英往復・設定130%/全文/無音・通常/reduced motion・再読み込みの日本語復帰を確認。
  実UIの身体13ノードで任意浮上（認識8）、エコー正答→Ω（認識10）、受動選択→未達ロック/浮上（認識0）。
  日本語の構造29ノード→Ω（認識12）、再降下cycle1、忘却後cycle0/認識0も確認。
  全通過点で320×568/130%の横overflowなし。入口・設定・結末を320×568/1280×800で画像確認。
  英語の全文表示は進行を変えない。未訳の構造本文と択の説明は日本語lang、未訳案内あり。
  offline reloadでも英語を選べる。不正な英語JSONでは英語を無効化し、日本語の開始/本文を維持。page error 0。
- backlog    : HZ-BL-020追加＝実装/agent検証済み・英語話者の試遊待ち。HZ-BL-018 / 001 / 002 / 012は継続。
  `docs/playtest/english-trial-e43.md`に対象・用語・英語の依頼文案・質問を記載し、E41の日本語自由試遊とは区別。
- next       : 了承を得た日本語初見テストと、英語話者による声/意味/門の公平さ/結末/待ち時間の確認。
- blockers   : 人間による訳の採否・面白さ・実機/耳の評価は未実施。募集/連絡/録画/外部収集は行っていない。
