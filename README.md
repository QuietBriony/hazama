...
`index.html` と `hazama-index.html` は `hazama-seed.js?v=2.16`、`hazama-state.js?v=2.16`、`hazama-main.js?v=2.16` の順に読み込みます。

## v2.16 Choice Language

各深度では、既存の選択肢ナビゲーションに加えて以下が動きます。

- 選択肢は「足元の光をたどって先へ進む」のような普通の行動文として表示する
- 安定や開度などのゲーム補足は、選択肢本文と分けて小さく表示する
- 初期/HUBでは探索者キービジュアルだけを枠内に出し、通常深度以降は曼荼羅ゲートを全画面背景に溶かす
- 深度や共鳴が進むほど、曼荼羅の存在感と回転・発光が少し強まる
- Gate Intelligence が現在の目的、危険度、推奨ルート、ゲート開度を提示する
- Gate Run が `さらに奥へ進む / 周囲をよく見る / 呼吸を整える / 扉に合わせる / 夜のハブへ戻る` の5行動でゲート開度100%を目指す
- Ω到達が本筋のゴール。Ωは `GATE OPEN` 後に入れる
- `GATE OPEN` で達成、安定切れやターン超過で `COLLAPSED` になり夜のハブへ戻る
- ゲート開度と危険度が、上部アニメーションの発光や圧力に反映される
- 深度・共鳴・安定・しるしから Music repo 用の UCM プロファイルを自動生成する
- 生成した Music JSON はコピー / ダウンロードできる
- `AUDIO.GATE` は Hazama profile payload を URL hash に載せて Music を開く
...
- `AUDIO.GATE` / dev `LOCAL` クリック時は、その選択先を優先して開く
- `AUDIO.GATE` は初期 / pending / blocked 時だけ小さく出し、`synced` 後は自動で畳む
- dev操作は localhost の drawer へ退避する
- Hazama起動後の最初のクリック / タップ / キー入力で `AUDIO.GATE` と同じMusic起動を試みる
- 短い問いを表示
- 1文だけ入力を受け取る
...
- `安定`: 深く進むほど減り、問いへの返答や夜のハブで回復する
- `共鳴`: 深度移動や返答で増え、上限に達すると `しるし` に結晶化する
- `しるし`: 節目到達や稀な返答で増え、深い選択肢の通行バッファになる
- `最深`: 到達した最深ランク
- `Gate Run`: gate開度、turn、last actionを保存し、Music payloadにも安全な集計値として渡す
...
- `index.html` が `hazama-main.js?v=2.16` を参照している
- `hazama-main.js` が `v2.16` で配信される
